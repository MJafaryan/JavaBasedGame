...
public class Mine extends Building implements Runnable {
    ...
    public Mine(Colony colony, Vector2 location, int height, int width, String mineName) throws Exception {
        super(colony, location, height, width, mineName, 3);
        JSONObject buildingInfo = (JSONObject) SimplerJson.getDataFromJson(configFile,
                String.format("farms_%s", mineName));
    ...
    public void upgrade() throws Exception {
        if (this.level < getMaxLevel()) {
            JSONObject newLevelInfo = (JSONObject) SimplerJson.getDataFromJson(configFile,
                    String.format("farms_%s_lvl%s", this.mineName, (this.level + 1)));
